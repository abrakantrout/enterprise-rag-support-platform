...
def run_tests():
    ...
    # The relative stored path is e.g. "uploads/uuid.pdf". Let's run ls/test command in container.
    filename_part = os.path.basename(stored_path)
    check_command = f"docker compose exec backend ls /workspace/uploads"
    print(f"Checking backend storage listing for file {filename_part}...")
    import subprocess
